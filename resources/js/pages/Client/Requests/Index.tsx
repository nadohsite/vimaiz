...
export default function Index({ requests, statuses }: Props) {
    const getStatusColor = (status: string) => {
        const colors: Record<string, string> = {
            ...
            quote_accepted: 'bg-emerald-100 text-emerald-800 border-emerald-200',
            quote_refused: 'bg-red-100 text-red-800 border-red-200',
                ...
                <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
                        <div>
                            ...
                            <p className="text-slate-500 mt-1">Suivez l'état de vos demandes de ménage</p>
                        </div>
                    ...
                    {requests.data.length > 0 ? (
                        <div className="space-y-4">
                            {requests.data.map((request) => (
                                <Link key={request.id} href={route('client.requests.show', request.id)}>
